...
func run() -> void:
	...
	await use()
	check(world.radio_tuner.active,"E opens physical radio tuner")
	await key(KEY_T)
	check(world.radio.enabled,"T powers radio from its tuner")
	var previous_frequency: float = world.radio.tuned_frequency
	await key(KEY_RIGHT)
	check(is_equal_approx(world.radio.tuned_frequency,previous_frequency+0.1),"Tuner changes frequency at radio")
